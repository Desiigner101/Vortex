...
public class Nyxarion_planetTransition implements Screen {
    // Transition constants
    private static final float PLANET_FINAL_X = -250f;
    private static final float PLANET_ZOOM_FACTOR = 0.8f;
    ...
    // Scary color scheme
    private static final Color BLOOD_RED = new Color(0.7f, 0.1f, 0.1f, 0.9f);
    private static final Color DARK_MAROON = new Color(0.3f, 0.0f, 0.0f, 0.9f);
    private static final Color SCARLET = new Color(1.0f, 0.2f, 0.2f, 1f);
    private static final Color ASH_GRAY = new Color(1f, 0.8f, 0.8f, 1f);

    private static final Color INFO_PANEL_COLOR = new Color(DARK_MAROON).sub(0, 0, 0, 0.15f);
    private static final Color INFO_TITLE_COLOR = new Color(SCARLET);
    private static final Color INFO_SUBTITLE_COLOR = new Color(ASH_GRAY);
    private static final Color INFO_TEXT_COLOR = new Color(0.9f, 0.9f, 0.9f, 1f);
    private static final Color PARTICLE_COLOR = new Color(BLOOD_RED).add(0.1f, 0, 0, 0);

    // Animation constants
    private static final float FRAME_DURATION = 0.1f;
    private static final int FRAME_COUNT = 48;
    private static final String IMAGE_PATH = "Backgrounds/worldBackGrounds_Images/nyxarion/NyxarionSprite";
    ...
    // Typewriter effect
    private final String fullTitle = "NYXARION";
    private final String fullDescription = "Blood-soaked wasteland of nightmares";
    ...
    private final String worldTitle = "Nyxarion";
    private final String worldSubtitle = "universe 3";
    ...
    private Texture createScanlineTexture() {
        ...
        pixmap.setColor(new Color(0.7f, 0, 0, 0.1f)); // Red scanlines
        pixmap.fill();
        pixmap.setColor(new Color(0.7f, 0, 0, 0.05f));
    ...
    private Particle createParticle() {
        ...
        // Blood red particles with variation
        p.color = new Color(
            BLOOD_RED.r * MathUtils.random(0.8f, 1.2f),
            BLOOD_RED.g * MathUtils.random(0.8f, 1.2f),
            BLOOD_RED.b * MathUtils.random(0.8f, 1.2f),
    ...
    private BitmapFont createTitleFont() {
        try {
            ...
            parameter.size = 85;
            parameter.color = SCARLET;
            parameter.borderColor = DARK_MAROON;
    ...
    private BitmapFont createMenuFont() {
        try {
            ...
            FreeTypeFontParameter parameter = new FreeTypeFontParameter();
            parameter.size = 30;
            parameter.color = INFO_TEXT_COLOR;
    ...
    private void createButtons() {
        ...
        // Create button background with blood red gradient
        ...
        for (int y = 0; y < pixmap.getHeight(); y++) {
            ...
            pixmap.setColor(new Color(
                DARK_MAROON.r * (1 - ratio) + BLOOD_RED.r * ratio,
                DARK_MAROON.g * (1 - ratio) + BLOOD_RED.g * ratio,
                DARK_MAROON.b * (1 - ratio) + BLOOD_RED.b * ratio,
                1f
            ));
        ...
        textButtonStyle.fontColor = SCARLET;
        ...
        viewButton.addListener(new InputListener() {
            ...
            public void enter(InputEvent event, float x, float y, int pointer, Actor fromActor) {
                viewButton.addAction(Actions.parallel(
                    Actions.color(new Color(1f, 0.3f, 0.3f, 1f), 0.3f),
    ...
    private void renderPlanet(float delta) {
        ...
        // Slower zoom using sine ease-in-out
        float easeProgress = progress < 0.5f ? 2f * progress * progress : -1f + (4f - 2f * progress) * progress;

        float baseScale = 4.0f;
        float scale = baseScale * (1 + easeProgress * PLANET_ZOOM_FACTOR); // Zooms more slowly

        float leftMargin = 370f;
        float initialX = Gdx.graphics.getWidth() - currentFrame.getWidth() * baseScale * 0.5f - leftMargin;
        float finalX = -1000; // Left side of the screen
        float xPos = initialX + (finalX - initialX) * easeProgress;
        ...
        if (easeProgress > 0.5f) {
            float zoomAmount = newWidth - currentFrame.getWidth() * scale;
            xPos -= zoomAmount; // Move the planet to the left so the right side zooms out
        }
        ...
        // Render planet with rotation centered
    ...
    private void renderOverlay() {
        ...
        // Main gradient fill with blood red colors
        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
        ...
        shapeRenderer.setColor(BLOOD_RED.r, BLOOD_RED.g, BLOOD_RED.b, glowAlpha * 0.3f);
        ...
        // Corner accents
        shapeRenderer.setColor(BLOOD_RED.r, BLOOD_RED.g, BLOOD_RED.b, glowAlpha);
    ...
    private void renderText() {
        ...
        titleFont.setColor(SCARLET.r, SCARLET.g, SCARLET.b, textAlpha);
        ...
        if (titleComplete) {
            ...
            subtitleFont.setColor(ASH_GRAY.r, ASH_GRAY.g, ASH_GRAY.b, textAlpha);
    ...
    private void renderInfoPanel() {
        ...
        // Border Fade-In Effect (Blood Red)
        if (wipeProgress < 1f) {
            Gdx.gl.glLineWidth(3);
            shapeRenderer.begin(ShapeRenderer.ShapeType.Line);
            shapeRenderer.setColor(BLOOD_RED.r, BLOOD_RED.g, BLOOD_RED.b, infoPanelAlpha);
            shapeRenderer.rect(leftEdge, panelY, currentHalfWidth * 2, panelHeight);
            shapeRenderer.end();
            Gdx.gl.glLineWidth(1);
        }

        // Decorative Lines and Corner Fade-In Effect (Blood Red)
        if (wipeProgress > 0.3f) {
            float decoAlpha = Math.min(1f, (wipeProgress - 0.3f) / 0.7f) * infoPanelAlpha;

            shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
            shapeRenderer.setColor(BLOOD_RED.r, BLOOD_RED.g, BLOOD_RED.b, decoAlpha);
        ...
        // Square Particles Fade-In Effect (Blood Red)
        if (wipeProgress < 1f) {
            ...
            for (int i = 0; i < 20; i++) {
                ...
                shapeRenderer.setColor(BLOOD_RED.r, BLOOD_RED.g, BLOOD_RED.b, particleAlpha);
    ...
    private void renderScanlines() {
        ...
        batch.begin();
        batch.setColor(0.7f, 0, 0, 0.15f); // Red scanlines
